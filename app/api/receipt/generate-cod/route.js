...
export async function POST(req) {
    try {
        ...
        // Check if order is COD and not cancelled
        if (order.paymentMethod !== 'cod' || order.status === 'cancelled') {
            return NextResponse.json(
                { success: false, message: "Receipt can only be generated for active COD orders" },
                { status: 400 }
            );
        }
        ...
        const receiptData = {
            ...
            paymentDetails: {
                ...
                paymentStatus: 'completed', // COD orders are completed when delivered
                paymentIntentId: null,
                transactionId: `COD-${order._id}`
            },
            financialSummary: {
                subtotal: subtotal,
                shipping: shipping,
        ...
        console.error("COD Receipt creation error:", error);
        return NextResponse.json(
